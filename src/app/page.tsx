...
const HomePage = () => {
  ...
  return (
    <main className="landing-background min-h-screen text-[#091329]">
      <ScienceWaterRipples />
      <div className="relative z-10 mx-auto w-full max-w-[1130px] px-5 pb-8 pt-6 sm:px-8 lg:px-12">
        ...
        <section className="grid gap-8 pb-12 pt-12 sm:gap-10 sm:pb-16 sm:pt-16 lg:grid-cols-[1.35fr_0.85fr] lg:items-center lg:gap-16 lg:pb-20">
          <div>
            ...
            <div className="mt-7 flex flex-wrap gap-2">{t.chips.map((item, index) => <span className={index === t.chips.length - 1 ? 'inline-flex w-full items-center justify-center rounded-xl border-2 border-[#79bdb6] bg-[#def2ef] px-5 py-3 text-[17px] font-black text-[#066b64] shadow-sm sm:w-auto' : 'rounded-full border border-[#dbe1eb] bg-white px-3 py-1.5 text-xs font-medium text-[#42506a]'} key={item}><span className="mr-2 text-[#0d8278]">●</span>{item}</span>)}</div>
          </div>
          ...
          <div className="rounded-[22px] border border-[#e0e5ed] bg-white p-5 shadow-[0_18px_45px_rgba(29,45,68,0.14)] sm:p-7" id="account">
            {authUser ? <div className="py-5 text-center"><span className="mx-auto flex size-12 items-center justify-center rounded-full bg-[#dfeeed] text-[#0d8278]"><Check /></span><h2 className="mt-4 text-xl font-bold">{t.welcome}</h2><p className="mt-2 text-sm text-[#71809b]">{authUser.email}</p><p className="mt-5 text-3xl font-black text-[#0d8278]">{tokenAccount?.tokenAvailable.toLocaleString() ?? '200,000'}</p><p className="mt-1 text-xs text-[#71809b]">{t.available}</p><button className="mt-6 text-sm font-semibold text-[#0d8278]" onClick={() => void handleLogout()} type="button">{t.logout}</button></div> : <>
              ...
              <div className="mt-4 flex gap-3 rounded-xl border-2 border-[#9bcfc9] bg-[#e5f2f1] p-4"><span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-white text-[#0d8278] shadow-sm"><ShieldCheck className="size-5" /></span><div><p className="text-[17px] font-black leading-5 text-[#075f59]">{t.noPhone}</p><p className="mt-1 text-[13px] leading-5 text-[#526f7d]">{t.noPhoneDetail}</p></div></div>
        ...
        <section className="pb-12 sm:pb-16"><h2 className="text-2xl font-black tracking-tight">{t.workspaceTitle}</h2><p className="mt-2 text-sm text-[#7b88a0]">{t.workspaceHint}</p><div className="mt-6 grid gap-5 md:grid-cols-2">{t.workspaces.map((workspace) => { const Icon = workspace.icon; return <Link className="group rounded-[18px] border border-[#dde3ec] bg-white p-6 transition hover:-translate-y-0.5 hover:border-[#9ccbc7] hover:shadow-lg sm:p-8" href={workspace.href} key={workspace.href}><span className="flex size-12 items-center justify-center rounded-xl bg-[#e1efee] text-[#0d8278]"><Icon className="size-6" /></span><h3 className="mt-5 text-xl font-black">{workspace.title}</h3><p className="mt-3 min-h-12 text-sm leading-6 text-[#58677f]">{workspace.description}</p><span className="mt-6 inline-flex items-center gap-1.5 rounded-full border border-[#b9d9d6] bg-[#f4faf9] px-3 py-1.5 text-xs font-bold text-[#08746c]"><Check className="size-3.5" />{t.included}</span></Link>; })}</div></section>
        <section className="flex flex-col gap-6 rounded-[22px] bg-[#0c6e66] px-7 py-8 text-white sm:flex-row sm:items-center sm:justify-between sm:px-11 sm:py-9"><div><h2 className="text-2xl font-black">{t.ctaTitle}</h2><p className="mt-2 max-w-[520px] text-sm leading-5 text-[#d7eeeb]">{t.ctaText}</p></div><button className="inline-flex w-full shrink-0 items-center justify-center gap-2 rounded-xl bg-white px-6 py-3 text-sm font-bold text-[#086b64] sm:w-auto" onClick={() => selectMode('signup')} type="button">{t.ctaButton}<ArrowRight className="size-4" /></button></section>
      </div>
    </main>
  );
};
