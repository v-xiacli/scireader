...
import { ArrowRight, BarChart3, Check, FileText, Gift, Loader2, MessageCircle, Sparkles, type LucideIcon } from 'lucide-react';
...
const HomePage = () => {
  ...
  return (
    <main className="ios-home landing-background min-h-screen text-[#091329]">
      <ScienceWaterRipples />
      <div className="ios-home-shell relative z-10 mx-auto w-full max-w-[1130px] px-4 pb-28 pt-4 sm:px-8 sm:pb-8 sm:pt-6 lg:px-12">
        <header className="ios-home-header flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2 text-xs font-medium">
            ...
            {authUser ? <>
              ...
              <button className="ios-mobile-hide-register rounded-lg border border-[#dce2ec] bg-white px-2.5 py-2 sm:px-4" onClick={() => selectMode('signup')} type="button">{language === 'zh' ? '注册' : 'Sign up'}</button>
              ...
              <button className="ios-mobile-guest-nav hidden rounded-full bg-white px-3 py-2 font-bold text-[#0b6e5a]" onClick={openGuestChat} type="button">{language === 'zh' ? '免注册对话' : 'Guest chat'}</button>
            </>}
          </nav>
        </header>

        <section className="ios-home-content grid gap-6 pb-9 pt-8 sm:gap-10 sm:pb-16 sm:pt-16 lg:grid-cols-[1.35fr_0.85fr] lg:items-center lg:gap-16 lg:pb-20">
          <div className="ios-hero-copy">
            <div className="ios-hero-badge inline-flex items-center gap-2 rounded-full bg-[#dfeeed] px-3 py-1.5 text-xs font-semibold text-[#08746c]"><Sparkles className="size-3.5" />{t.badge}</div>
            <h1 className="ios-hero-title mt-5 max-w-[620px] text-[36px] font-black leading-[1.08] tracking-[-0.04em] sm:text-[50px] sm:leading-[1.05]">{t.headline}</h1>
            <p className="ios-hero-intro mt-5 max-w-[590px] text-[15px] leading-7 text-[#62708a]">{t.intro}</p>
            <ul className="ios-feature-list mt-6 space-y-3 text-[14px] text-[#34425c]">{t.benefits.map((item) => <li className="flex items-center gap-3" key={item}><span className="flex size-5 shrink-0 items-center justify-center rounded-md bg-[#dceeed] text-[#0d8278]"><Check className="size-3.5" strokeWidth={3} /></span>{item}</li>)}</ul>
            <button className="ios-mobile-guest-cta hidden w-full items-center gap-3" onClick={openGuestChat} type="button"><span className="flex size-[52px] shrink-0 items-center justify-center rounded-2xl bg-[#0b6e5a] text-white"><MessageCircle className="size-6" /></span><span className="min-w-0 flex-1 text-left"><span className="block text-[19px] font-black">{language === 'zh' ? '免注册对话' : 'Chat without signup'}</span><span className="mt-1 block text-xs opacity-70">{language === 'zh' ? '3,000 免费 tokens · 支持直接上传照片' : '3,000 free tokens · Upload photos directly'}</span></span><ArrowRight className="size-5 shrink-0" /></button>
            <div className="ios-chip-list mt-7 flex flex-wrap gap-2">{t.chips.map((item, index) => <span className={index === t.chips.length - 1 ? 'inline-flex w-full items-center justify-center rounded-2xl border-2 border-[#79bdb6] bg-[#def2ef] px-6 py-4 text-[24px] font-black leading-tight text-[#066b64] shadow-sm sm:w-auto sm:text-[27px]' : 'rounded-full border border-[#dbe1eb] bg-white px-3 py-1.5 text-xs font-medium text-[#42506a]'} key={item}><span className="mr-3 text-[#0d8278]">●</span>{item}</span>)}</div>
          </div>

          <div className="ios-account-card rounded-[18px] border border-[#e0e5ed] bg-white p-4 shadow-[0_12px_32px_rgba(29,45,68,0.12)] sm:rounded-[22px] sm:p-7 sm:shadow-[0_18px_45px_rgba(29,45,68,0.14)]" id="account">
            {authUser ? <div className="py-1 text-center sm:py-5"><span className="mx-auto flex size-10 items-center justify-center rounded-full bg-[#dfeeed] text-[#0d8278] sm:size-12"><Check className="size-5 sm:size-6" /></span><h2 className="mt-2 text-lg font-bold sm:mt-4 sm:text-xl">{t.welcome}</h2><p className="mt-1 truncate text-xs text-[#71809b] sm:mt-2 sm:text-sm">{authUser.email}</p><p className="mt-3 text-[28px] font-black leading-none text-[#0d8278] sm:mt-5 sm:text-3xl">{tokenAccount?.tokenAvailable.toLocaleString() ?? '200,000'}</p><p className="mt-1 text-xs text-[#71809b]">{t.available}</p><button className="mt-3 text-sm font-semibold text-[#0d8278] sm:mt-6" onClick={handleLogout} type="button">{t.logout}</button></div> : <>
              ...
              <button className="ios-account-guest-cta mt-4 flex w-full items-center gap-4 rounded-xl border-2 border-[#0d8278] bg-[#e5f2f1] p-4 text-left transition hover:bg-[#d8efec]" onClick={openGuestChat} type="button"><span className="flex size-12 shrink-0 items-center justify-center rounded-lg bg-white text-[#0d8278] shadow-sm"><MessageCircle className="size-6" /></span><span className="min-w-0 flex-1"><span className="block text-[22px] font-black leading-7 text-[#075f59]">{language === 'zh' ? '免注册对话' : 'Chat without signup'}</span><span className="mt-1 block text-[13px] leading-5 text-[#526f7d]">{language === 'zh' ? '无需登录 · 3,000 免费 tokens · 可以直接上传照片' : 'No login · 3,000 free tokens · Upload photos directly'}</span></span><ArrowRight className="size-5 shrink-0 text-[#0d8278]" /></button>
              <div className="ios-mobile-email-note hidden items-start gap-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-[10px] bg-white text-[#0b6e5a]"><Check className="size-5" /></span><span><span className="block text-[15px] font-bold">{language === 'zh' ? '无需手机号，仅使用邮箱注册' : 'Email registration only'}</span><span className="mt-1 block text-xs leading-5 opacity-80">{language === 'zh' ? '无需手机验证、短信验证码或信用卡' : 'No phone verification, SMS, or credit card'}</span></span></div>
              {authMessage ? <p className="mt-3 text-center text-xs text-rose-600">{authMessage}</p> : null}
              <button className="mt-4 flex h-12 w-full items-center justify-center rounded-xl border-2 border-[#b9d9d6] bg-white text-sm font-extrabold text-[#08746c] transition hover:border-[#0d8278] hover:bg-[#f4faf9]" onClick={() => setAuthMode(authMode === 'signup' ? 'login' : 'signup')} type="button">{authMode === 'signup' ? (language === 'zh' ? '登录' : 'Log in') : (language === 'zh' ? '注册账号' : 'Create account')}</button>
            </>}
          </div>
        </section>

        <section className="ios-workspaces pb-9 sm:pb-16"><h2 className="text-[21px] font-black leading-tight tracking-tight sm:text-2xl">{t.workspaceTitle}</h2><p className="mt-2 text-[13px] leading-5 text-[#7b88a0] sm:text-sm">{t.workspaceHint}</p><div className="mt-4 grid gap-3 sm:mt-6 sm:gap-5 md:grid-cols-2">{t.workspaces.map((workspace) => { const Icon = workspace.icon; return <Link className="ios-workspace-card group rounded-[16px] border border-[#dde3ec] bg-white p-4 transition hover:-translate-y-0.5 hover:border-[#9ccbc7] hover:shadow-lg sm:rounded-[18px] sm:p-8" href={workspace.href} key={workspace.href}><span className="flex size-10 items-center justify-center rounded-xl bg-[#e1efee] text-[#0d8278] sm:size-12"><Icon className="size-5 sm:size-6" /></span><h3 className="mt-3 text-lg font-black sm:mt-5 sm:text-xl">{workspace.title}</h3><p className="mt-2 text-[13px] leading-5 text-[#58677f] sm:mt-3 sm:min-h-12 sm:text-sm sm:leading-6">{workspace.description}</p><span className="mt-4 inline-flex items-center gap-1.5 rounded-full border border-[#b9d9d6] bg-[#f4faf9] px-3 py-1.5 text-xs font-bold text-[#08746c] sm:mt-6"><Check className="size-3.5" />{t.included}</span></Link>; })}</div></section>
      </div>
    </main>
  );
};
